...
> **Có hai gói: một cho máy Windows, một cho MacBook.** Gói Windows là bốn nút `.bat` kèm sẵn Node; gói Mac (`Tool_nhap_lieu_mac.zip`) là bốn nút `.command` và **máy Mac phải cài Node một lần** — xem mục *MacBook: ba việc khác Windows* ngay dưới. Việc hằng ngày của hai bản giống hệt nhau. Bản bấm nút ngay trong Google Sheet (không cài gì) vẫn nằm trong kế hoạch.
...
### MacBook: ba việc khác Windows

Gói cho Mac tên **`Tool_nhap_lieu_mac.zip`**, bốn nút đuôi **`.command`** (bấm đúp y như `.bat`). Khác đúng ba chỗ:

1. **Cài Node.js một lần.** Vào <https://nodejs.org>, tải bản **LTS** cho macOS (file `.pkg`), bấm đúp rồi bấm Tiếp tục tới hết. Không cài thì nút 1 dừng lại và in đúng ba bước này ra màn hình.
2. **Lần đầu bấm nút, macOS sẽ chặn** vì file tải từ Internet: hiện câu *"không thể mở vì không rõ nhà phát triển"*. Cách mở: **chuột phải vào nút → Mở → Mở**. Chỉ phải làm một lần cho mỗi nút. (Không thấy chữ *Mở* thì vào **Cài đặt hệ thống → Quyền riêng tư & Bảo mật**, kéo xuống cuối, bấm **Vẫn mở**.)
3. **Lần đầu tool đọc thư mục thả file**, macOS hỏi xin quyền vào Desktop/Documents — bấm **Cho phép**, không thì tool không thấy file bạn vừa thả.

Sau ba việc đó thì mọi thứ y hệt bản Windows: thả file vào đúng thư mục gian hàng rồi bấm đúp **`4_CHAY_TOOL.command`**.
