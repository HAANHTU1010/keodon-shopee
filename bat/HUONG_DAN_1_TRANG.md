...
> **Có hai gói: một cho máy Windows, một cho MacBook.** Gói Windows là bốn nút `.bat`, gói Mac (`Tool_nhap_lieu_mac.zip`) là bốn nút `.command` — **cả hai đều kèm sẵn Node, không phải cài gì**. Máy Mac chỉ khác đúng một thao tác lần đầu (mục *MacBook: khác Windows đúng một chỗ* ngay dưới). Việc hằng ngày của hai bản giống hệt nhau. Bản bấm nút ngay trong Google Sheet vẫn nằm trong kế hoạch.
...
### MacBook: khác Windows đúng một chỗ

Gói cho Mac tên **`Tool_nhap_lieu_mac.zip`**, bốn nút đuôi **`.command`** (bấm đúp y như `.bat`). **Không phải cài Node** — gói đã kèm sẵn.

1. **Lần đầu bấm nút, macOS chặn** vì file tải từ Internet: hiện câu *"không thể mở vì không rõ nhà phát triển"*.
   Cách mở: **chuột phải vào `1_CAI_DAT_LAN_DAU.command` → Mở → Mở**. Chỉ làm một lần: chính nút đó tự gỡ cờ chặn cho cả gói, ba nút còn lại bấm đúp chạy thẳng.
   (Không thấy chữ *Mở* thì vào **Cài đặt hệ thống → Quyền riêng tư & Bảo mật**, kéo xuống cuối, bấm **Vẫn mở**.)
2. **Lần đầu tool đọc thư mục thả file**, macOS hỏi xin quyền vào Desktop/Documents — bấm **Cho phép**, không thì tool không thấy file bạn vừa thả.

Sau đó mọi thứ y hệt bản Windows: thả file vào đúng thư mục gian hàng rồi bấm đúp **`4_CHAY_TOOL.command`**.

> **MacBook đời cũ chạy chip Intel** (máy trước 2020): gói kèm sẵn Node cho máy Apple Silicon thôi, nên máy Intel sẽ thấy nút 1 in ba bước cài Node từ <https://nodejs.org> (chọn bản **LTS**, file `.pkg`). Cài xong bấm lại nút 1 là chạy tiếp bình thường.
...
- **Đơn hủy / hoàn / chưa chốt tiền vẫn được ghi, nhưng TÔ VÀNG.** Tool đẩy lên đủ đơn đủ số như báo cáo rồi tô vàng dòng đó và ghi lý do vào cột `Note` ("SOÁT TAY: …") — ví dụ đơn hủy, đơn đã hoàn tiền toàn bộ, đơn khách đang đòi trả hàng, đơn TikTok chưa tính phí. **Bạn soát mấy dòng vàng đó rồi sửa tay** (xóa dòng, sửa số lượng, sửa tiền). Chạy lại tool **không bao giờ sửa dòng đã ghi**, nên phần bạn chỉnh tay không bị đè.
- **Lấy đúng số tiền TikTok trả về.** Cột `Tổng Tiền SP` = cột **`Số tiền quyết toán ước tính`** trong file TikTok (đơn nhiều dòng thì cộng lại) — đúng con số bạn vẫn gõ tay. Ba cột `MGG Shop`, `Chi phí`, `Thuế` **để trống** như cách bạn vẫn nhập, nên `Doanh Thu` = `Tổng Tiền SP`. Tool không tự tính phí hay thuế. Vài đơn có thể chênh vài nghìn so với số trên màn hình đơn hàng của Seller Center (màn hình tách riêng phí SFR) — số trong file mới là số TikTok trả thật.
...
| `… ĐƠN CẦN SOÁT TAY` | Không phải lỗi. Đơn hủy / hoàn / chưa chốt tiền đã được ghi và **tô vàng**; đọc cột `Note` rồi sửa tay mấy dòng đó |
| `BỎ QUA … ĐƠN KHÔNG PHẢI ĐƠN BÁN` | Giao dịch không phải đơn hàng (tiền quảng cáo, điều chỉnh…) — không có tên hàng nên không ghi được thành dòng |
| `KHÔNG CÓ FILE MỚI` | Chưa thả file, hoặc file lần trước đã sang `đã xử lý` |
| `TU_KIEM_LECH` | Các cột tiền trong báo cáo không cộng ra đúng `Số tiền quyết toán ước tính` (TikTok có thể đã đổi cách ghi) — tool **không ghi đơn TikTok nào**. Gửi file cho người phụ trách, trong lúc chờ nhập tay như cũ |
...
Khi người phụ trách báo có bản mới, bấm đúp **`2_CAP_NHAT.bat`** (máy Mac: **`2_CAP_NHAT.command`**), đợi vài giây, đọc dòng cuối.
