...
- Trước khi làm, nó in lại bảy giá trị và hỏi `Dung chua? (c/k)`. Gõ `c` mới làm.
- Chế độ 1 tự kiểm tám phép so trước khi báo xong. **Lệch một phép là dừng, không ghi link.**
...
| `DA CAP NHAT XONG.  2.5.0  ->  2.6.0` | Xong rồi. Bấm `4_CHAY_TOOL.bat` làm việc như mọi ngày |
